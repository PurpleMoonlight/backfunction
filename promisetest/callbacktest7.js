...
async function main(){
    try{
        /* //使用for循环来代替繁多的语句
        [6,5,6].forEach(async (cHander)=>{
            const cHanderResult=await cdfuntion(cHander)
            console.log(cHanderResult)});
         */
        //使用并发操作执行这些任务  
        const cHanderResult1=cdfuntion(6);
        const cHanderResult2=cdfuntion(5);
        const cHanderResult3=cdfuntion(6);
        const [r1,r2,r3]  =await  Promise.all([cHanderResult1,cHanderResult2,cHanderResult3])  
        console.log(r1);    
        console.log(r2);
        console.log(r3);
    }   
